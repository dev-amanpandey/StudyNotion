...
const formatDuration = (seconds) => {
  ...
  return [
    hours && `${hours}h`,
    minutes && `${minutes}m`,
    ((!hours && !minutes) || remainingSeconds) && `${remainingSeconds}s`,
  ].filter(Boolean).join(" ")
}
...
function CourseDetails() {
  ...
  useEffect(() => {
    ...
    response?.data?.courseContent?.forEach((sec) => {
      lectures += sec?.subSections?.length ?? sec?.subSection?.length ?? 0
    })
...
// console.log("paymentLoading:", paymentLoading)
  ...
  if ( !response) {
    console.log("❌ RETURNING SPINNER: response is missing")
  ...
  if (!response?.success || !response?.data) {
      console.log("❌ RETURNING ERROR COMPONENT")
  console.log("success =", response?.data?.success)
  console.log("data =", response?.data?.data)

    return <Error />
  }
  //console.log("✅ REACHED COURSE UI")
  ...
  const totalDuration = (courseContent || []).reduce(
    (courseSeconds, section) =>
      courseSeconds + (section?.subSections || section?.subSection || []).reduce(
        (sectionSeconds, lecture) => sectionSeconds + getDurationInSeconds(lecture?.timeDuration),
        0
      ),
    0
  )
            ...
            >
              ...
              <div className="flex flex-wrap gap-5 text-lg">
                <p className="flex items-center gap-2">
                  {" "}
                  <BiInfoCircle /> Created at {formatDate(createdAt || course_id)}
                </p>
                <p className="flex items-center gap-2">
                  {" "}
